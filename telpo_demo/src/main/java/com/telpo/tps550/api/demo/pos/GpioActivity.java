...
public class GpioActivity extends Activity {
    ...
    private void initView() {
        ...
        if(CommonUtil.getGPIOStatus(mGpioType)[0] == CommonConstants.GPIODirection.INPUT){
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.btn_gpio_in:
                btn_gpio_low.setEnabled(false);
                btn_gpio_high.setEnabled(false);
                Toast.makeText(this, "" + CommonUtil.setGPIOControl(mGpioType, CommonConstants.GPIODirection.INPUT), Toast.LENGTH_SHORT).show();
                break;
            case R.id.btn_gpio_out:
                btn_gpio_low.setEnabled(true);
                btn_gpio_high.setEnabled(true);
                Toast.makeText(this, "" + CommonUtil.setGPIOControl(mGpioType, CommonConstants.GPIODirection.OUTPUT), Toast.LENGTH_SHORT).show();
                break;
            case R.id.btn_gpio_low:
                Toast.makeText(this, "" + CommonUtil.setGPIOLevel(mGpioType, CommonConstants.GPIOLevel.LOW_LEVEL), Toast.LENGTH_SHORT).show();
                break;
            case R.id.btn_gpio_high:
                Toast.makeText(this, "" + CommonUtil.setGPIOLevel(mGpioType, CommonConstants.GPIOLevel.HIGH_LEVEL), Toast.LENGTH_SHORT).show();
                break;
            case R.id.btn_gpio_check:
                int[] arr = CommonUtil.getGPIOStatus(mGpioType);
